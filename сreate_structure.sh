...
docker exec -it MySQL mysql -u root -p rainbow -e 'CREATE TABLE Costs (Id INT NOT NULL primary key AUTO_INCREMENT,Name VARCHAR(255),Price decimal(6,1));CREATE TABLE Products (Id SERIAL NOT NULL primary key,Name VARCHAR(255),Status VARCHAR(255),Quantity INT,PriceId int, FOREIGN KEY (PriceId)  REFERENCES Costs(Id));'
